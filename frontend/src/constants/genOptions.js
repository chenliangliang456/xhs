...
export function resolveColorOption(colorKey, slotIndex, seed) {
  if (colorKey === 'random') {
    return pickBySeed(PICKABLE_COLORS, hashSlot(seed, slotIndex));
  }
  return COLOR_OPTIONS.find((c) => c.id === colorKey) || PICKABLE_COLORS[0];
}

export function resolveShapeOption(shapeCategory, shapeKey, slotIndex, seed) {
  const pool =
    shapeKey === 'random'
      ? ALL_SHAPES
  ...
  if (shapeKey === 'random') {
    return pickBySeed(pool, hashSlot(seed, slotIndex + 101));
  }
  return ALL_SHAPES.find((s) => s.id === shapeKey) || REGULAR_SHAPES[0];
}

export function resolveStyleOption(styleKey, slotIndex, seed) {
  if (styleKey === 'random') {
    return pickBySeed(PICKABLE_STYLES, hashSlot(seed, slotIndex + 202));
  }
  return STYLE_OPTIONS.find((s) => s.id === styleKey) || STYLE_OPTIONS[0];
}

export function resolveTextColorOption(textColorKey) {
  return TEXT_COLOR_OPTIONS.find((c) => c.id === textColorKey) || TEXT_COLOR_OPTIONS[0];
}

export function resolveFontOption(fontKey) {
  return FONT_OPTIONS.find((f) => f.id === fontKey) || FONT_OPTIONS[0];
}

export function resolveGenModifiers(options = {}, slotIndex = 0, seed = 0) {
...
export function buildPromptSuffix(modifiers) {
  const parts = [];
...
export function buildFullPrompt(basePrompt, modifiers) {
...
export function formatGenTags(modifiers) {
  if (!modifiers) return '';
  return [
    modifiers.colorLabel,
    modifiers.shapeLabel,
    modifiers.styleLabel,
    modifiers.textColorLabel && modifiers.fontLabel
      ? `文字${modifiers.textColorLabel}·${modifiers.fontLabel}`
      : null,
  ]
    .filter(Boolean)
    .join(' · ');
