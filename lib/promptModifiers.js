...
function buildCreativeCBreakdownHints(stylePrompt) {
  const style = stylePrompt ? `整体视觉调性：${stylePrompt}；` : '';
  return [
    '以锚点 A 同款种子纸为唯一实物主体，创作一张「活字印刷风格的产品视觉拆解标注图」：',
    style,
    '【禁止】信息海报、说明书、传单、满版文字、密集段落、厚重信息卡片、上下分栏详情页、多张产品拼图、宫格分格、大面积留白、纯白背景；',
    '【构图】画面正中央仅放置一张完整种子纸（与锚点同款、同色、同设计），微俯拍或平铺；',
    '【背景】必须沿用锚点 A 图的背景场景（同色调、同光影、同材质），标注与背景自然融合；',
    '【印刷质感】标注文字须呈现活字印刷/活版印刷/数码印刷工艺感（铅字压印、油墨微凸、纸纤维吃墨、轻微印痕）；',
    '从中心产品向四周发散多条极细引导线（annotation line），线末端连接小型精炼中文标签，标签短句化、点状呈现；',
    '产品仍是绝对主角，标注线细而优雅，不抢主体。',
  ].join('');
}
